...
SELECT val, ROW_NUMBER() OVER(ORDER BY val) AS rownum FROM T ORDER BY val;
...
ORDER BY orderdate, orderid) AS prev_diff,

LEAD(qty) OVER(PARTITION BY custid
ORDER BY orderdate, orderid) AS next_diff
FROM dbo.Orders;
